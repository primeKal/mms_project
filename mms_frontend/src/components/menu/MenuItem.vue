<template>
    <div class="h-28 w-full flex bg-slate-50 rounded overflow-hidden">
        <img :src="menuItem.image" class="w-1/3 object-cover object-center" />
        <div class="p-2 flex flex-col">
            <p>{{ menuItem.name }}</p>
            <p class="mt-2 font-medium">{{ menuItem.price }}</p>
            <div class="w-full flex justify-between">
                <div class="flex">
                    <button @click="quantity > 0 ? quantity-=1 : quantity" class="py-1 px-2 text-primary border border-primary rounded-l shadow-sm">
                        -
                    </button>
                ...
                <div class="w-min">
                    ...
                    >
                        <button @click="toogleCart(true)" v-if="!addedToCart" class="py-1 px-2 flex items-center rounded border border-primary text-primary active:scale-95">
