    ...
    <div class="h-28 w-full flex bg-[#F8F8FC] rounded-lg border overflow-hidden">
        ...
        <div class="w-full py-1 px-3 flex flex-col">
            <div class="w-full flex justify-between">
                ...
                <svg v-if="showCloseIcon" @click="toogleCart(false)" class="cursor-pointer text-red-400"
            ...
            <div class="mt-2 w-full flex justify-between">
                <div class="flex items-center">
                    <button @click="updateQuantityAndAddToCart(false)"
                        class="py-1 px-2 text-primary border border-primary rounded-l shadow-sm">
                        <!-- minus icon svg -->
                    ...
                    <button @click="updateQuantityAndAddToCart(true)"
                        class="py-1 px-2 text-primary border border-primary rounded-r shadow-sm">
                        <!-- plus icon svg -->
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24">
                ...
                <div v-if="!showCloseIcon" class="ml-5">
                    <transition mode="out-in" enter-from-class="scale-0 rotate-180" enter-to-class="scale-100 rotate-0"
                        enter-active-class="transition-transform duration-300 ease-in"
                        leave-from-class="scale-100 rotate-0" leave-to-class="scale-0 rotate-180"
                        leave-active-class="transition-transform duration-300 ease-out">
                        <button @click="toogleCart(true)" v-if="!addedToCart"
                            class="py-1 px-2 flex items-center rounded border border-primary text-primary font-medium active:scale-95">
...
export default {
    ...
    methods: {
        ...mapMutations({
            addCart: 'Order/addCartItem',
            removeFromCart: 'Order/removeItemFromCart',
        }),
        ...
        updateQuantityAndAddToCart(direction) {
            if (this.saveOnClick) {
                if (direction) {
                    this.quantity += 1
                } else {
    ...
    mounted() {
        this.quantity = this.menuItem?.quantity || 1
    }
}
</script>