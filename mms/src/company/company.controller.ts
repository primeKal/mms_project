...
export class CompanyController {
  ...
  @UseInterceptors(FileInterceptor('company_img', multerOptionsCompanyImg))
  @Post()
  public async createCompany(
    @UploadedFile() company_img,
  ...
  @UseGuards(JwtAuthGuard)
  @Put(':id')
  public async editCompany(@Body() body: UpdateCompanyDto, @Param('id') id): Promise<Company> {
    return this.companyService.editCompany(body, id);
  }
    ...
    FileInterceptor('file', {
      storage: diskStorage({
        ...
        filename: (req: any, file: Express.Multer.File, cb): void => {
          ...
          const newFileName =
            req.user.id + '_' + Date.now() + '.' + file_extension;
          cb(null, newFileName);
        },
      }),
