    ...
    <div class="lg:py-3 py-0 flex lg:flex-col flex-row">
        ...
        <div class="lg:mt-8 mt-2 lg:w-auto w-full flex lg:flex-col flex-row overflow-x-scroll lg:snap-none snap-x snap-proximity touch-pan-x text-gray-800">
            <router-link 
                ...
                :class="[nav.id === navValue ?'lg:border-l-blue-700 border-b-blue-700 bg-blue-200' : 'lg:border-l-white border-b-white']"
            >
                <Icon class="lg:block hidden" :icon="nav.icon" />
                <label :class="[collapse ? 'hidden': 'block']" class="ml-2 lg:text-base text-sm text-nowrap">{{ nav.name }}</label>
            </router-link>
        </div>
    </div>
</template>
