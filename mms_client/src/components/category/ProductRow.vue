<template>
    ...
    <div v-if="!editProduct" class="flex justify-between w-full py-1 lg:pl-6 pl-2">
        ...
        <p class="lg:basis-1/5 basis-1/4 text-black/60 font-medium">{{ product?.name }}</p>
    ...
    <div v-else class="flex justify-between w-full py-1 lg:pl-6 pl-2">
        <p class="basis-auto">{{index+1}}</p>
        ...
        <td class="pl-1 py-2 flex justify-center items-center lg:text-base text-sm">
            <button @click="editProduct = false" class="py-1 lg:px-4 px-2 text-gray-400 hover:text-gray-500 active:scale-95 transition-all font-semibold rounded">cancel</button>
            <button @click="updateProduct" class="py-1 lg:px-4 px-2 text-white bg-green-400 hover:bg-green-500 active:scale-95 transition-all font-semibold rounded">Save</button>
        </td>
    </div>
</template>
<script>
