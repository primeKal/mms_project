<template>
    <DeleteConfirmationModalVue v-if="openDeleteCategory" :title="openDeleteCategory.name" :id="openDeleteCategory.id"
        @deleteItem="deleteSection" @closeModal="openDeleteCategory = null" />

    <tr :class="openProducts ? 'shadow-md bg-primary text-white' : 'bg-white text-black shadow-none hover:bg-slate-50/60'"
        ...
        <td class="pl-4 py-3 flex items-center font-light">
            <svg class="mr-3" @click="openProducts = !openProducts" :class="[openProducts ? 'rotate-180' : 'rotate-0']"
        ...
        <td class="pl-2 py-3 font-light truncate">
            ...
            <textarea v-else style="resize:none;" class="border rounded h-20 p-1"
                v-model="sectionInfo.description"></textarea>
        </td>
        <td v-if="!editSection" class="pl-4 py-3 flex justify-center items-center">
            <svg @click="editSection = true"
            ...
            <svg @click="openDeleteCategory = category"
        ...
        <transition enter-from-class="h-0" enter-to-class="h-auto" enter-active-class="transition-all"
