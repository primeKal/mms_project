<template>
    <div>
        <table class="w-full border-collapse">
            ...
            <transition enter-from-class="h-0" enter-to-class="h-auto" enter-active-class="transition-all duration-500"
                leave-from-class="h-auto" leave-to-class="h-0" leave-active-class="transition-all duration-500">
                <tr v-if="openNewCategory" class="grid grid-cols-3 bg-slate-100 px-4 py-2">
                    ...
                    <div class="col-span-full w-full grid grid-cols-3 gap-4 bg-white p-3 rounded">
                        <input class="h-min w-full border rounded py-1 pl-2" placeholder="Section name"
                            v-model="newCategory.name" />
                        <textarea style="resize:none;" class="h-20 rounded p-1 border" placeholder="Descripiton"
                        ...
                        <div class="flex justify-center items-center space-x-3 pl-4 py-1">
                            ...
                            <button @click="$emit('closeNewCategory')"
                                class="py-1 px-4 text-white bg-gray-400 hover:bg-gray-500 active:scale-95 transition-all font-semibold rounded">cancel</button>
                        </div>
                    </div>
                </tr>
            </transition>
            ...
            <tbody>
                <CategoryRowVue v-for="category in categories" :key="category.id" :category="category"
