<template>
    <MainRendererVue :loading="loading">
        <QRCodeModalVue v-if="openQRView" :companyInfo="openQRView" :qrValue="'https://www.google.com'"
            @closeModal="() => openQRView = null" />

        <DeleteConfirmationModalVue v-if="openDelete" :title="openDelete.name" :id="openDelete.id"
            @deleteItem="deleteMenu" @closeModal="openDelete = null" />

        <div class="p-6 pr-12 w-full">
            <h3 class="text-primary font-medium text-xl">Menu List</h3>
            <div class="mt-4 w-full flex justify-between">
                <SearchBarVue class="w-2/3" :placeholderString="'Search menus'" @searchChanged="searchCategories" />
                <button @click="openNewMenu = true" :disabled="openNewMenu"
            ...
            <table class="mt-6 w-full">
                ...
                <tbody>
                    <!-- create menu start -->
                    <transition enter-from-class="h-0" enter-to-class="h-auto"
                        enter-active-class="transition-all duration-500" leave-from-class="h-auto" leave-to-class="h-0"
                        ...
                        <tr v-if="openNewMenu">
                            ...
                            <td class="flex justify-center items-center space-x-3 pl-4 py-2">
                                <button @click="openNewMenu = false"
                                    class="py-1 px-4 text-gray-400 hover:text-gray-500 active:scale-95 transition-all font-semibold rounded">cancel</button>
                                <button @click="createMenu"
                                    class="py-1 px-4 text-white bg-green-400 hover:bg-green-500 active:scale-95 transition-all font-semibold rounded">Save</button>

                            </td>
                        </tr>
                    </transition>
                    <!-- create menu end -->
                    <tr v-for="menu in menus" :key="menu.id" class="hover:bg-slate-50/60 border-b transition-colors">
                        <td class="pl-4 py-3 font-light">{{ menu.id }}</td>
                        <td class="pl-4 py-3 font-light">{{ menu.name }}</td>
                        <td :class="[menu.isActive ? 'text-green-400' : 'text-red-600']" class="pl-4 py-3 font-light">{{
                            menu.isActive }}</td>
                        <td class="pl-4 py-3 flex justify-center items-center">
                            <svg @click="setQRinfo(menu)"
    ...
    methods: {
        ...
        async createMenu() {
            ...
            if (this.newMenu.name.length > 0) {
                const status = await this.$store.dispatch('Menu/addNewMenu', this.newMenu)
                if (status.success) {
