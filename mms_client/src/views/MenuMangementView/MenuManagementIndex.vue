<template>
    <MainRendererVue :loading="loading">
        ...
        <div class="lg:p-6 p-2 lg:pr-12 pr-2 w-full">
            ...
            <div class="mt-4 w-full flex lg:flex-row flex-col lg:justify-between">
            <SearchBarVue 
                    class="lg:w-2/3 w-full"
                    :placeholderString="'Search menus'" 
                    @searchChanged="searchCategories"
            />
                <button @click="openNewMenu = true" :disabled="openNewMenu" class="lg:w-1/5 w-1/3 lg:mt-0 mt-3 lg:self-auto self-end bg-transparent hover:bg-primary text-primary hover:text-white border-primary rounded border active:scale-95 transition-all">
                    + NEW MENU
                </button>
            </div>
            <table class="mt-6 w-full">
                <thead class="bg-gray-100 border-y shadow-sm lg:text-base text-sm">
                ...
                <tbody>
                    ...
                    >
                        <tr v-if="openNewMenu" class="lg:text-base text-xs">
                            <td class="pl-4 py-1">New</td>
                            <td class="pl-4 py-2">
                                <input class="py-1 pl-1 rounded border lg:text-base text-xs" v-model="newMenu.name" />
                            </td>
                            <td class="lg:block hidden"></td>
                            <td class="flex justify-center items-center lg:space-x-3 space-x-1 lg:pl-4 pl-1 py-2">
